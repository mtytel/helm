...
TwytchComputerKeyboard::TwytchComputerKeyboard(mopo::TwytchEngine* synth,
...
TwytchComputerKeyboard::~TwytchComputerKeyboard() {
}

void TwytchComputerKeyboard::changeKeyboardOffset(int new_offset) {
...
bool TwytchComputerKeyboard::keyPressed(const KeyPress &key, Component *origin) {
  return false;
}

bool TwytchComputerKeyboard::keyStateChanged(bool isKeyDown, Component *originatingComponent) {
  bool consumed = false;
  ScopedLock lock(*critical_section_);
  for (int i = 0; i < strlen(KEYBOARD); ++i) {
    int note = computer_keyboard_offset_ + i;
